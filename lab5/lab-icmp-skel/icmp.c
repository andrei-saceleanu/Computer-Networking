...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for(i=0;i<count;i++){
		...
		icmp_hdr->un.echo.id = htons(getpid());//htons(getpid() & 0xFFFF);
		...
		printf("Ping number: %d ", i);
        printf("Response type: %x ", ricmp_hdr->type);
        printf("Response code : %x ", ricmp_hdr->code);
        printf("Response checksum: %x ", ricmp_hdr->checksum);
        printf("Sequence number: %d ", htons(ricmp_hdr->un.echo.sequence));
        printf("Receiver socket ID: %x ", ricmp_hdr->un.echo.id);
        printf("Response time: %0.4f ", time_elapsed);
		printf("Replier : %s \n",inet_ntoa(*(struct in_addr *)&rip_hdr->saddr));
	}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while(1){
		...
		get_interface_ip(sockfd,IFNAME,&(ip_hdr->saddr));
		//ip_hdr->saddr = get_default_gateway_ip();
		ip_hdr->daddr = ip_addr.s_addr;
		ip_hdr->check = 0;
		ip_hdr->protocol = IPPROTO_ICMP;
		ip_hdr->check = checksum(ip_hdr,sizeof(struct iphdr));
		...
		icmp_hdr->type = ICMP_ECHO;
		icmp_hdr->checksum = 0;
		icmp_hdr->un.echo.sequence = cnt;
		icmp_hdr->un.echo.id = htons(getpid());
		icmp_hdr->checksum = checksum(icmp_hdr,sizeof(struct icmphdr));

		struct packet reply;
		time_packet(sockfd, &pkt, &reply);

		struct iphdr *rip_hdr = (struct iphdr *)(reply.payload + IP_OFF);
		struct icmphdr *ricmp_hdr = (struct icmphdr *)(reply.payload + ICMP_OFF);
		if(ricmp_hdr->type!=0){
			printf("Hop: %d Address: %s \n",cnt,inet_ntoa(*(struct in_addr *)&rip_hdr->saddr));
		}else{
			printf("Reached %s with limit: %d\n",inet_ntoa(*(struct in_addr *)&rip_hdr->saddr),cnt);
			break;
		}
		cnt++;
	}	
