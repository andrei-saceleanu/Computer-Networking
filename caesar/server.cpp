...
unordered_map<string,status> stats;
unordered_map<int,string> sock_to_user;
unordered_map<string,bool> ok;



int check_caesar(char *text,char *cipher){
	char *p = text;
	char *q = cipher;
	if(strlen(text)!=strlen(cipher))
		return 0;
	for(int i=0;i<strlen(text);i++){
		int diff = fabs(text[i]-cipher[i]);
		if(diff!=13)
			return 0;
	}
	return 1;
				...
				if (i == sockfd) {
					...
					if (n == 0) {
						...
						if(buffer[0]=='l'){
							...
							ok[user] = true;
							if(stats.find(user)==stats.end()){
								stats.insert({user,new_status});
								stats[user].enc = 0;
								stats[user].dec = 0;
								stats[user].ver = 0;
							}
							...
							cout<<strlen(v->text)<<" "<<strlen(v->cipher)<<"\n";
							if(check_caesar(v->text,v->cipher)){
								memset(response,0,BUFLEN);
								sprintf(response,"Criptare corespunzatoare\n");
								send(i,response,BUFLEN,0);
							}else{
								memset(response,0,BUFLEN);
								sprintf(response,"Textul si cifrul nu corespund\n");
								send(i,response,BUFLEN,0);
							}
							stats[sock_to_user[i]].ver++;
							//printf("%s\n", buffer);
						}else if(buffer[0]=='e'){
							if(!ok[sock_to_user[i]]){
								memset(response,0,BUFLEN);
							...
							memset(buffer, 0, BUFLEN);
							n = recv(i, buffer, sizeof(buffer), 0);
							encdec_command *ed = (encdec_command *)buffer;
							for(int i=0;i<strlen(ed->text);i++){
								unsigned char c = ed->text[i]+13;
								ed->text[i] = (c <= 122) ? c : (c-26);
							}
							send(i,(char *)ed,sizeof(encdec_command),0);
							stats[sock_to_user[i]].enc++;
						}else{
							//cout<<i<<" "<<sock_to_user[i]<<"\n";
							if(!ok[sock_to_user[i]]){
								memset(response,0,BUFLEN);
